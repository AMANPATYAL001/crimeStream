...
# inputT.to_csv('a.csv')
# inputT=pd.read_csv('onlyEn.csv').head(300)
# inputT=pd.Series(['l hate you','i love you',' i will kill you'])
# inputT=pd.Series([tweet])
...
# inputT = inputT.apply(lambda x: preprocess(x))

# tokenizer = Tokenizer()
# tokenizer.fit_on_texts(inputT.text)
# tokenizer.fit_on_texts(inputT)

# from tensorflow import keras
# model=keras.models.load_model('res/model12v1.h5')
...
# final_df=pd.merge(inputT,neg_df,how='right',right_index=True,left_index=True).sort_values('score',ascending=False)
